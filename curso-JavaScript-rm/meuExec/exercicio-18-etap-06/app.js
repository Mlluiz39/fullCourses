...
inputElement.addEventListener('keyup', (event) => {
  const inpurRegexp = /^[a-zA-Z]{6,}$/;
  const resultInpuRegexp = inpurRegexp.test(event.target.value);

  paragraph.setAttribute('data-feedback', 'feedback-username');

  const feedbackParagraph = document.querySelector(
    '[data-feedback="feedback-username"]'
  );
...
form.addEventListener('submit', (event) => {
  ...
  const formvalue = event.target.username.value;
  const inpurRegexp = /^[a-zA-Z]{6,}$/;
  const resultInpuRegexp = inpurRegexp.test(formvalue);
  const paragraph = document.createElement('p');

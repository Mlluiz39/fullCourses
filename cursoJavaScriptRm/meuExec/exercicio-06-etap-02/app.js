...
const myAge = 41;
let conditionToEnter = null;
const isChild = myAge <= 7;
const isOlder = myAge >= 65;

if (isChild || isOlder) {
  conditionToEnter = "Para você, a entrada é grátis!";
} else {
  conditionToEnter = "A entrada é R$ 30,00.";
}
...
for (let i = 0; i < numbers.length; i++) {
  if (numbers[i] >= 11 && numbers[i] <= 90) {
    selectedArray.push(numbers[i])
  }
}
codigo abaixo refatorado:
*/

for (let i = 0; i < numbers.length; i++) {
  const number = numbers[i];
  const isSelectedArray = number >= 11 && number <= 90;

  if (isSelectedArray) {
    selectedArray.push(number);
  }
}
...
for (let i = 0; i < crazyArray.length; i++) {
  ...
  const typeofCrazyArrayNumber = typeofItem === "number";
  const typeofCrazyArrayBoolean = typeofItem === "boolean";

  if (typeofCrazyArrayNumber) {
    numberAmount++;
  } else if (typeofCrazyArrayBoolean) {
    booleanAmount++;
  } else {
    stringAmount++;
  }
...
let oddRandomNumbers = []
let evenRandomNumbers = []

for (let i = 0; i < randomNumbers.length; i++) {
 if (randomNumbers[i] % 2 === 0) {
   evenRandomNumbers.push(randomNumbers[i])
 } else {
   oddRandomNumbers.push(randomNumbers[i])
 }
}

console.log(`Numeros ímpares: ${oddRandomNumbers.join(', ').replace('31', 'e 31')}. Números pares: ${evenRandomNumbers.join(', ').replace('58', 'e 58')}.`)
 """"""""""""""""""""""""""""""""""""codigo abaixo refatorado""""""""""""""""""""""""      */
...
for (let i = 0; i < randomNumbers.length; i++) {
  ...
  if (isEvenNumber) {
    evenRandomNumbers.push(number);
  } else {
    oddRandomNumbers.push(number);
  }
}

const evenNumbersString = evenRandomNumbers.join(", ").replace("58", "e 58");
const oddNumbersString = oddRandomNumbers.join(", ").replace("31", "e 31");
